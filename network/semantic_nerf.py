...
class Semantic_NeRF(nn.Module):
    ...
    """
    def __init__(self, enable_semantic, num_semantic_classes, D=8, W=256, input_ch=3, input_ch_views=3, input_ch_feat=32, output_ch=4, skips=[4], use_viewdirs=True,
                 num_rays=32*32, num_samples=32, chunk_size=[240//4, 320//4], patch_size=3, is_chunk=False):
    ...
    def forward(self, x, show_endpoint=False):
        ...
        for i, l in enumerate(self.pts_linears):
            h_feat = self.pts_linears[i](h_feat) * bias
            h_feat = F.relu(h_feat)
            if i in self.skips:
                h_feat = torch.cat([input_pts, h_feat], -1)

        if self.use_viewdirs:
            # if using view-dirs, output occupancy alpha as well as features for concatenation
            alpha = self.alpha_linear(h_feat)
            if self.enable_semantic:
                sem_logits = self.semantic_linear(h_feat)
            feature = self.feature_linear(h_feat)
